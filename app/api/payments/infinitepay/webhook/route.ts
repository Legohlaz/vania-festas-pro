...
export async function POST(request: NextRequest) {
  ...
  if (!infinitePayAmountMatches(verification.amount, Number(attempt.amount))) return error("Valor do pagamento não corresponde à reserva.", 409);

  const paymentMethod = infinitePayMethod(verification.capture_method);
